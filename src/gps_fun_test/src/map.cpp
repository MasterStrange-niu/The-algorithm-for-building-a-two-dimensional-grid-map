...
bool calculate_obstacle_location(double lat, double lon,double distance1, double angle1,double& lat_obstacle_first, double& lon_obstacle_first,double& lat_obstacle_second,double& lon_obstacle_second) {
    for(int i =0;i<2;i++){
          // 将激光雷达的极坐标转换为直角坐标
      ...
      if(i==0){
            // 第一次计算障碍物的经纬度
        lon_obstacle_first = obstacle_x;
        lat_obstacle_first = obstacle_y;
      }else{
        lon_obstacle_second = obstacle_x;
        lat_obstacle_second = obstacle_y;
      }
    }
      if(lon_obstacle_first==lon_obstacle_second&&lat_obstacle_first==lon_obstacle_second){
        return true;
      }
      else{
        return false;
  ...
  void zhong_location(double lat, double lon,
    ...
    // 将激光雷达的极坐标转换为直角坐标
    double zhong_x = -sqrt(2)*ROWS/2;
    double zhong_y = sqrt(2)*COLS/2;
...
//calculateDistances(dist, bear, north_south_dist, east_west_dist);
    ...
    double lat_obstacle_first;
    double lon_obstacle_first;
    double lat_obstacle_second;
    double lon_obstacle_second;
    double bear;
    double dist;

  if(calculate_obstacle_location( lat,  lon,  distance1,  angle1,  lat_obstacle_first, lon_obstacle_first,lat_obstacle_second,lon_obstacle_second)){
    calculate_obstacle_location( lat,  lon,  distance1,  angle1,  lat_obstacle_first, lon_obstacle_first,lat_obstacle_second,lon_obstacle_second);
    distance( lat3,  lon3, lat_obstacle_first, lon_obstacle_first, dist) ;
    bearing(lat3,  lon3,  lat_obstacle_first,  lon_obstacle_first, bear) ;
    calculateDistances(dist, bear, north_south_dist, east_west_dist);
    //所有打印语句
    ...
    //遍历所有栅格，将有障碍物的变黑
      for(int i = 0; i < ROWS; i++) {
        for(int j = 0; j < COLS; j++) {
            if(i == north_south_dist  && j == east_west_dist) {  
